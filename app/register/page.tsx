...
export default function Register() {
  ...
  const passwordStrength = () => {
    if (password.length === 0) return { width: "0%", color: "bg-border" };
    if (password.length < 6) return { width: "25%", color: "bg-destructive" };
    if (password.length < 8) return { width: "50%", color: "bg-yellow-500" };
    if (password.length < 10) return { width: "75%", color: "bg-primary/70" };
    ...
    <main className="min-h-screen bg-background bg-mesh relative overflow-hidden flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8 sm:mb-10">
          ...
          <p className="mt-3 text-sm text-muted-foreground">Join the arena</p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-surface/50 backdrop-blur-sm p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              {password.length > 0 && (
                <div className="mt-2">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    ...
                    {password.length >= 6 && password.length < 8 && "Could be stronger"}
                    {password.length >= 8 && password.length < 10 && "Getting better"}
            ...
            <div>
              ...
              <input
                ...
                placeholder="••••••••"
                required
                className={`w-full px-4 py-2.5 rounded-lg border bg-background text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-mono text-sm ${
                  confirmPassword && password !== confirmPassword ? "border-destructive" : "border-border"
                }`}
              />
            ...
            <label className="flex items-start gap-2 text-sm text-muted-foreground cursor-pointer">
              <input
                ...
                className="mt-1 rounded border-border bg-background focus:ring-primary/20 text-primary"
                required
              />
              <span>
                I agree to the{" "}
            ...
            <button
              type="submit"
              disabled={!agreedToTerms || (confirmPassword !== "" && password !== confirmPassword)}
              className="w-full py-3 rounded-lg bg-primary text-primary-foreground font-semibold tracking-wide hover:pulse-glow transition-all text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-border bg-background text-foreground hover:border-primary/50 hover:bg-surface transition-all text-sm cursor-pointer">
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          ...
          <Link href="/login" className="text-primary hover:text-primary/80 transition-colors font-medium cursor-pointer">
            Sign in
          </Link>
        </p>
